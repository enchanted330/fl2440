...
int main (int argc, char **argv)
{
    ...
    strncpy(sqlcmd, "create table if not exists cplus(ID integer primary key autoincrement, name nvarchar(32))", sizeof(sqlcmd));
    ...
    snprintf(sqlcmd, sizeof(sqlcmd), "insert into cplus values(1, '%s')", "guowenxue");
    if(SQLITE_OK != (rv=sqlite3_exec(db, sqlcmd, NULL, NULL, &errmsg)) )
    {
    ...
    snprintf(sqlcmd, sizeof(sqlcmd), "insert into cplus values(2, '%s')", "jason");
    if(SQLITE_OK != (rv=sqlite3_exec(db, sqlcmd, NULL, NULL, &errmsg)) )
    {
        printf("Add item into cplus failed [%d]: %s\n", rv, errmsg);
        sqlite3_free(errmsg);
    ...
    snprintf(sqlcmd, sizeof(sqlcmd), "insert into cplus values(3, '%s')", "adrian");
    if(SQLITE_OK != (rv=sqlite3_exec(db, sqlcmd, NULL, NULL, &errmsg)) )
    {
        printf("Add item into cplus failed [%d]: %s\n", rv, errmsg);
        sqlite3_free(errmsg);
